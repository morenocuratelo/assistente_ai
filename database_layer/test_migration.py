        ...
        if backup_path and os.path.exists(backup_path):
            ...
            with sqlite3.connect(test_db_path) as conn:
                ...
                try:
                    ...
                    print(f"⚠️ Projects dopo rollback: {projects_after_rollback} (dovrebbe essere 0)")

                    # Verifica project_id rimosso dalle tabelle
                    cursor.execute("PRAGMA table_info(papers)")
                    columns = [col['name'] for col in cursor.fetchall()]
                    has_project_id = 'project_id' in columns
                    print(f"⚠️ Papers ha project_id dopo rollback: {has_project_id} (dovrebbe essere False)")

                except sqlite3.Error:
                    print("✅ Rollback completato - schema originale ripristinato")
